...
def plot_feature_importance_for_class(model, X_train):
    ...
    shap.summary_plot(shap_values, X_train, plot_type="bar", class_names=model.classes_, color=pl.get_cmap("tab10"), show=False) #labels=model.classes_
    ...
    plt.savefig('feature_importance_for_class.png')
...
def plot_prediction_desicion(model, X_test, pred, row_idx):
    ...
    shap.multioutput_decision_plot([1, 2, 3], shap_values,
                                   ...
                                   show=False)
    plt.savefig('prediction_decision.png')
